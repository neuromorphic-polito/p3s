...
void acp_delay_modifier(uint16_t varcode, uint8_t *data, uint16_t length);
...
void c_main(void) {
    ...
    // ACPv5 is not compatible for now - disabled

    // Setup the SDP callback of ACP
    // Not use ACP Init if use simulation_sdp_callback_on
//    simulation_sdp_callback_on(ACP_SDP_PORT, acp_sdp_callback);
//    acp_variable_init();
//    acp_init();
//    acp_me_create(10, sizeof(uint8_t), acp_delay_modifier, NULL);
//    acp_variable_register(10, sizeof(uint8_t), acp_delay_modifier, NULL);
